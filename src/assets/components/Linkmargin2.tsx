...
export type Linkmargin2Type = {
  camaroteSeTuDixCarnaval?: string;
  floripa2024?: string;
...
const Linkmargin2: FunctionComponent<Linkmargin2Type> = ({
  camaroteSeTuDixCarnaval,
  floripa2024,
  propAlignSelf,
}) => {
  const diveventTitleStyle: CSSProperties = useMemo(() => {
    return {
  ...
  return (
    <div className="h-full w-[317px] shrink-0 flex flex-row items-start justify-center pt-0 px-0 pb-4 box-border max-w-[317px] min-h-[326px] max-h-[326px] ml-[-4px] text-left text-mid-2 text-darkslategray font-www-blueticket-com-br-1680x783-default-inter-medium-1356">
      <div className="h-full w-[285px] rounded-lg bg-white shadow-[0px_2px_8px_rgba(25,_43,_70,_0.08)] flex flex-col items-start justify-start max-w-[285px] min-h-[310px] max-h-[310px]">
        ...
        <div className="self-stretch rounded-t-none rounded-b-lg flex flex-col items-start justify-between">
          <div
            className="flex flex-col items-start justify-start py-4 pr-6 pl-3"
            style={diveventTitleStyle}
          >
            <div className="relative leading-[21px] font-medium inline-block max-w-[261px]">
              <p className="m-0">{camaroteSeTuDixCarnaval}</p>
              <p className="m-0">{floripa2024}</p>
            </div>
          </div>
